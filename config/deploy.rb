...
require File.join(RAILS_ROOT + '/lib', 'personal_information.rb')
...
set :application, PersonalInformation::AptanaApplicationName
...
set :user, PersonalInformation::AptanaAccountUsername
set :password, PersonalInformation::AptanaAccountPassword
...
set :db_password, PersonalInformation::AptanaDatabasePassword

after "deploy:restart", "turksurveyor_custom"
task :turksurveyor_custom, :roles => :app, :except => {:no_release => true, :no_symlink => true} do
 run "cd public/web/current && rake RAILS_ENV=production asset:packager:build_all"
end
